...
pub const String = packed struct {
    len: i32,
    payload: packed union(u96) {
        content: u96,
        heap: packed struct(u96) { prefix: u32, ptr: usize },
    },
    ...
    pub fn deinit(self: *const String, allocator: Allocator) void {
        const len = self.len;
        if (len > 12) {
            allocator.free(heapPtr(self)[0..@intCast(len)]);
        }
    }
    ...
    pub fn eql(a: String, b: String) bool {
        ...
        return std.mem.eql(u8, heapPtr(&a)[0..al], heapPtr(&b)[0..bl]);
    }
